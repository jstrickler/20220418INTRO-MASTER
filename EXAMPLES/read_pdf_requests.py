...
saved_pdf_file = 'nasa_iss.pdf'  # <2> 739318main_ISS Utilization Brochure 2012 Screenres 3-8-13.pdf

response = requests.get(url, auth=('username', 'l0lz'), params={'key': 'mykey...'})  # <3>
if response.status_code == requests.codes.OK:  # <4>
    if response.headers.get('content-type') == 'application/pdf':
        with open(saved_pdf_file, 'wb') as pdf_in:  # <5> "Download as"
